...
## Implemented MVP
- Student roster management with validation, duplicate-contact protection, age/date validation and active/inactive status
- Lesson scheduling with overlap protection, future-date validation, rescheduling, status updates and date-range viewing
- Curriculum / progress tracking using an ordered hourglass-style skills list
- Invoicing with payment terms, cancellation rules, payment recording/history, payment methods/references, overdue status and outstanding-balance reporting
...
## Business rules
The core layer enforces important domain rules rather than relying only on the console UI. These include: inactive students cannot receive new lessons or invoices; student email/phone duplicates are rejected; dates of birth cannot be future dates; new and rescheduled lessons must be in the future; scheduled lessons cannot overlap; completed/cancelled/no-show lessons cannot be reopened as scheduled; only scheduled lessons can be rescheduled; cancelled invoices are excluded from outstanding balances and cannot receive payments; paid or partially paid invoices cannot be cancelled; payment dates cannot be future dates or before the invoice issue date; payment amounts cannot exceed the invoice balance; and payment method/reference information is persisted.
